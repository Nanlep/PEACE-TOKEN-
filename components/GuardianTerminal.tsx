...
const GuardianTerminal: React.FC<GuardianTerminalProps> = ({ 
  ...
  const handleUnlockGate = (e: React.FormEvent) => {
    e.preventDefault();
    // Production Note: In a real system, this would be an OIDC/Auth0 or hardware key challenge.
    // For this mission-critical simulation, we use a Protocol Access Key.
    if (accessCode.toUpperCase() === 'PEACE-ADMIN-2025') {
      setIsGateUnlocked(true);
      setError(null);
    } else {
      setError('AUTHORIZATION_FAILURE: INVALID_ACCESS_KEY');
      setAccessCode('');
    }
  };
  ...
  const handleVerifySession = () => {
    ...
    setIsVerifying(true);
    // Simulated Secure Handshake (HSM / Hardware Key Verification)
    setTimeout(() => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      {!isGateUnlocked ? (
        /* Layer 1: Access Gate */
        <div className="glass-panel w-full max-w-md p-8 rounded-3xl border border-amber-500/30 relative animate-in zoom-in-95 duration-300 shadow-[0_0_50px_rgba(245,158,11,0.2)]">
          <div className="flex flex-col items-center text-center space-y-6">
            ...
            <div>
              <h2 className="text-xl font-black text-amber-500 uppercase tracking-widest">Security Challenge</h2>
              <p className="text-[10px] text-slate-500 font-mono uppercase tracking-[0.2em] mt-1">Personnel Authorization Required</p>
            </div>

            <form onSubmit={handleUnlockGate} className="w-full space-y-4">
              <div className="space-y-1">
                <label className="block text-[9px] font-black text-slate-500 uppercase text-left pl-1">Protocol Access Key</label>
                <input 
                  autoFocus
                  type="password"
                  value={accessCode}
                  ...
                  placeholder="••••••••••••"
                  className="w-full bg-black/60 border border-white/10 rounded-xl px-4 py-3 text-white text-center font-mono focus:border-amber-500 outline-none transition-all"
                />
                {error && <p className="text-[9px] text-rose-500 font-bold animate-pulse mt-2">{error}</p>}
              </div>
              <button className="w-full py-4 bg-amber-600 hover:bg-amber-500 text-black font-black text-[10px] uppercase tracking-[0.3em] rounded-xl transition-all shadow-xl shadow-amber-900/20">
                Authenticate Enclave
              </button>
              <p className="text-[8px] text-slate-600 font-mono uppercase italic">Attempt will be logged to the Immutable Ledger (0x..F12)</p>
            </form>
          </div>
        </div>
      ) : (
        /* Layer 2: Main Terminal Interface (Unlocked) */
